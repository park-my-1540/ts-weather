...
export default function Widget({ items, weatherRef }: WidgetProps) {
  ...
  const moveToSub = useCallback(() => {
    ...
    if (weatherRef.current) {
      ...
      const scaleValue = Math.max(0, 1 - gap / 22);
      // scale을 gap을 기준으로 반비례적으로 설정 gap이 커질수록 scale은 작아짐
    ...
    <Position position="absolute" top="40%" right="30%">
      {weatherItem && (
        <motion.div animate={widgetControl}>
          <Text sizes="large" color="accent" textAlign="center">
            {weatherItem.main}
            <IconText
              icon={setWeatherIcon(weatherItem.main)}
              fontSize="80px"
              color="accent"
              style={{ marginLeft: 8 }}
            />
